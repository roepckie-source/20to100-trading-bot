...
def init_exchanges():
    ...
    # --------------------------------------------------------
    # OKX
    # Unterstützt beide möglichen Secret-Namensvarianten.
    # --------------------------------------------------------
    okx_key = os.getenv("OKX_API_KEY")
    okx_sec = os.getenv("OKX_SECRET") or os.getenv("OKX_API_SECRET")
    okx_pass = os.getenv("OKX_PASSWORD") or os.getenv("OKX_PASSPHRASE")
    ...
    mexc_sec = os.getenv("MEXC_SECRET") or os.getenv("MEXC_API_SECRET")
    ...
    bit_sec = os.getenv("BITRUE_SECRET") or os.getenv("BITRUE_API_SECRET")
    ...
    for symbol in SYMBOLS_TO_SCAN:
        ...
        for buy_ex_name, sell_ex_name in pairs:
            ...
            if spread_pct > 0.05:
                print(
                    f"🔍 {symbol} | "
                    f"Buy [{buy_ex_name.upper()} @ "
                    f"${buy_price:.4f}] ➔ "
                    f"Sell [{sell_ex_name.upper()} @ "
                    f"${sell_price:.4f}] | "
...
def execute_arbitrage_trade(
    ...
    try:
        ...
        print(
            ...
            f"Order-ID: {buy_order.get('id')}"
        )

    except Exception as e:

        print(
            f"❌ [{buy_name}] Kauf fehlgeschlagen: {e}"
        )
    ...
    try:
        ...
        print(
            f"⚠️ Coin muss über Auto-Cleanup "
            f"verkauft werden."
        )
...
def run_trader():
    ...
    print(
        f"\n✅ {len(exchanges)} Börsen erfolgreich initialisiert!"
    )
    ...
    print(
        f"Ordergröße: {TRADE_AMOUNT_USDT:.2f} USDT"
    )
    ...
    print(
        f"Maximale Laufzeit: "
        f"ca. {MAX_CYCLES * SCAN_INTERVAL_SECONDS / 60:.1f} Minuten"
    )
